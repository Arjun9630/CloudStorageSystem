      ...
      {isOpen && (<>
          ...
          <aside className="lg:hidden fixed top-0 left-0 bottom-0 w-64 bg-white z-50 flex flex-col">
            ...
            <nav className="flex-1 p-4 space-y-1 overflow-y-auto">
              {navItems.map((item) => (<NavLink key={item.path} to={item.path} end={item.path === '/'} onClick={() => setIsOpen(false)} className={({ isActive }) => `flex items-center gap-3 px-4 py-2.5 rounded-lg transition-colors ${isActive
            ...
            <div className="p-4 space-y-4 border-t border-gray-200">
              <div className="bg-gray-50 rounded-lg p-4">
                ...
                <div className="w-full bg-gray-200 rounded-full h-2">
                  <div className="bg-blue-600 h-2 rounded-full transition-all" style={{ width: `${Math.min(storagePercent, 100)}%` }}/>
                </div>
