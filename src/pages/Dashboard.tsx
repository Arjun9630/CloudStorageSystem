    ...
    return (
      <ul className="flex flex-wrap justify-center gap-x-6 gap-y-2 mt-2">
        {payload.map((entry: any, index: number) => {
          ...
          return (
            <li key={`item-${index}`} className="flex items-center gap-2 text-sm text-gray-700 font-medium">
          ...
          {storageByType.length > 0 ? (
            <ResponsiveContainer width="100%" height={320}>
              <PieChart>
                ...
                <Tooltip formatter={(value: number) => formatBytes(value)} contentStyle={{ borderRadius: '12px', border: 'none', boxShadow: '0 4px 6px -1px rgb(0 0 0 / 0.1)' }} />
        ...
        {recentFiles.length > 0 ? (
          <div className="space-y-3">
            {recentFiles.map((file) => (
              <div key={file.id} className="flex items-center gap-4 p-4 hover:bg-white/10 rounded-xl transition-all border border-transparent hover:border-white/10">
                {file.thumbnail ? (
                  <img src={file.thumbnail} alt="" className="w-12 h-12 object-cover rounded-xl shadow-[0_4px_12px_rgba(0,0,0,0.3)] border border-white/10" />
                ) : (
                  <div className="w-12 h-12 bg-black/20 rounded-xl flex items-center justify-center border border-white/10 shadow-inner">
